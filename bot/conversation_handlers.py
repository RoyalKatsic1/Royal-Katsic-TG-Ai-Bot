...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        text="Hi it's Royal Katsic Ai . Feel free to ask me anything and talk to me about what you want",
...
async def start_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    message_content = "You asked for a conversation. Royal Katsic Ai is free Let's start conversation!"

    conv_id = context.user_data.get("conversation_id")
    if conv_id:
        message_content = "You asked for a continue conversation. Royal Katsic Ai is free OK, Let's go!"
...
async def start_image_conversation(
    ...
    msg = await query.edit_message_text(
        f"You asked for Image description. OK, Send your image to Royal Katsic Ai with caption!",
        reply_markup=reply_markup,
    )
    context.user_data["to_delete_message"] = msg
...
def restricted(func):
    ...
    async def wrapped(update, context, *args, **kwargs):
        ...
        if user_id != int(os.getenv("AUTHORIZED_USER")):
            ...
            await update.message.reply_animation(
                "https://github.com/sudoAlireza/GeminiBot/assets/87416117/beeb0fd2-73c6-4631-baea-2e3e3eeb9319",
                caption="This is my persoanl GeminiiBot, to run your own Bot look at:\nhttps://github.com/sudoAlireza/GeminiBot",
            )
            return
        return await func(update, context, *args, **kwargs)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text(
        text="Hi it's Royal Katsic Ai. You can ask me anything and talk to me about what you want",
        reply_markup=reply_markup,
    )
